...
class TFModel(nn.Module):
    def __init__(self, img_size=64, patch_size=1, in_chans=3, out_chans=3, fuse_c=16,
        ...
        for i_layer in range(num_layers):
            layer = RTFL(dim=embed_dim,
                         ...
                         qkv_bias=qkv_bias, qk_scale=qk_scale,
                         drop=drop_rate, attn_drop=attn_drop_rate,
                         drop_path=dpr3[sum(depths[3][:i_layer]):sum(depths[3][:i_layer + 1])],  # no impact on SR results
                         norm_layer=norm_layer,
                         downsample=None,
                         use_checkpoint=use_checkpoint,
        ...
        # self.conv_after_body0 = nn.Sequential(nn.Conv2d(embed_dim+fuse_c, embed_dim, 3, 2, 1),
        #                                       nn.LeakyReLU(negative_slope=0.2, inplace=True),
        #                                       nn.Conv2d(embed_dim, embed_dim, 3, 1, 1),
        #                                       nn.LeakyReLU(negative_slope=0.2, inplace=True),)
        # self.conv_after_body1 = nn.Sequential(nn.Conv2d(embed_dim+fuse_c*2, embed_dim, 3, 2, 1),
        #                                       nn.LeakyReLU(negative_slope=0.2, inplace=True),
        #                                       nn.Conv2d(embed_dim, embed_dim, 3, 1, 1),
        #                                       nn.LeakyReLU(negative_slope=0.2, inplace=True),)
        # self.conv_after_body2 = nn.Sequential(nn.Conv2d(embed_dim+fuse_c*4, embed_dim, 3, 2, 1),
        #                                       nn.LeakyReLU(negative_slope=0.2, inplace=True),
        #                                       nn.Conv2d(embed_dim, embed_dim, 3, 1, 1),
        #                                       nn.LeakyReLU(negative_slope=0.2, inplace=True),)
        ...
        # self.conv_up16 = nn.Sequential(nn.ConvTranspose2d(embed_dim, embed_dim, 3, 1, 1, output_padding=0),
        #                               nn.LeakyReLU(negative_slope=0.2, inplace=True))
        # self.conv_up32 = nn.Sequential(nn.ConvTranspose2d(embed_dim, embed_dim, 3, 2, 1, output_padding=1),
        #                               nn.LeakyReLU(negative_slope=0.2, inplace=True))
    ...
    def forward(self, x0, x1,iter=1):

        # x0_0 = self.conv_first(x0.contiguous())  # 1
        # x0_1 = self.conv_first(x1.contiguous())
        
        # fea0_0 = self.forward_features(x0_0, self.layers0)
        # fea0_1 = self.forward_features(x0_1, self.layers0)
    
        # x1_0 = self.conv_after_body0(fea0_0)  # 1->1/2
        # x1_1 = self.conv_after_body0(fea0_1)
        # fea1_0 = self.forward_features(x1_0, self.layers1)
        # fea1_1 = self.forward_features(x1_1, self.layers1)
        

        # x2_0 = self.conv_after_body1(fea1_0)  # 1/2->1/4
        # x2_1 = self.conv_after_body1(fea1_1)  # 1/2->1/4
        # fea2_0 = self.forward_features(x2_0, self.layers2)
        # fea2_1 = self.forward_features(x2_1, self.layers2)

        # x3_0 = self.conv_after_body2(fea2_0)
        # x3_1 = self.conv_after_body2(fea2_1)

        # fea32_0 = self.forward_features(x0, self.layers2)
        # fea32_1 = self.forward_features(x1, self.layers2)
        # if iter == 1:
        #     return fea32_0,fea32_1
       
        if iter == 0:
            fea16_0 = self.forward_features(x0, self.layers3)
            fea16_1 = self.forward_features(x1, self.layers3)
            # fea16_0 = self.conv_up16(fea16_0) + x3_0 
            # fea16_1 = self.conv_up16(fea16_1) + x3_1
            return fea16_0, fea16_1
        if iter == 1:
            fea32_0 = self.forward_features(x0, self.layers2)
            fea32_1 = self.forward_features(x1, self.layers2)
            return fea32_0, fea32_1
